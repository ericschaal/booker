                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                           ...
                           System.out.println("Lock on TrxnId: " + xid + "was set to be converted");

                           // remove the old locks from the table
                        ...
                        //add new locks to the table wether we upgrade or not
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {
                ...
                if (dataObj.getLockType() == DataObj.READ) {
                    ...
                    //(1): we already have  a read lock
                    if(dataObj2.getLockType() == DataObj.READ){


                        // look for other members of the lock table with a lock on the object
                        for(Object o : vect){
                            if(((DataObj) o).getXId() != dataObj.getXId()){
                                System.out.println("[DEBUG] Want WRITE, someone has READ");
                                return true;
                            }
                        }

                        // set the lock for conversion
                        bitset.set(0,true);
                    }

                    //(2): transaction already has a write lock
                    // The lock request is reduntant since transaction already has a write lock
